...
const HeroSection = ({ showContent = true }: Props) => (
  <section className="relative min-h-screen flex flex-col items-center justify-end px-6 pb-16">
    <video
      ...
      loop
      playsInline
      className="absolute inset-0 w-full h-full object-cover transition-all duration-[4s] ease-out"
      style={{
    ...
    <div
      className="absolute inset-0 bg-background/40 z-[1] transition-opacity duration-[4s] ease-out"
      style={{ opacity: showContent ? 1 : 0 }}
    />

    <div
      className="relative z-20 flex flex-col items-center gap-2 max-w-3xl mx-auto text-center transition-all duration-[3s] ease-out"
      style={{
    ...
    <div
      className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-background to-transparent z-10 transition-opacity duration-[4s] ease-out"
      style={{ opacity: showContent ? 1 : 0 }}
    />
  </section>
);
